class AppsController < ApplicationController
  ...
  def update
    ...
    current_app.update!(plan_id: plan.id)
    # Update plan on stripe
    old_item_id = Stripe::Subscription.retrieve(current_app.stripe_subscription_id).items.data.first.id
    price_id = ENV["#{params[:guest]}_PRODUCT"]
    Stripe::SubscriptionItem.create({ subscription: current_app.stripe_subscription_id, price: price_id, proration_behavior: 'create_prorations' })
